...
if __name__ =='__main__':
    ...
    data = torch.load("PolitiFact_news_data_probs_llama_new.pt")
    word_probs = data['probs'].float()
    labels = data['labels'].long()

    # 数据集划分
    X_train, X_test, y_train, y_test = train_test_split(word_probs, labels, test_size=0.93, random_state=42)
    ...
    # 初始化用于保存最佳模型的变量
    best_accuracy = 0.0
    best_model_path = 'GPT3.5_best_model.pth'
    print("*"*80)
    print("Training...")
    # 训练模型
